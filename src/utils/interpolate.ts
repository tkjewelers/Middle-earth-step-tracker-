...
export function getPositionOnPath(milesTraveled: number): SegmentPosition {
  for (const segment of JOURNEY_SEGMENTS) {
    ...
    if (milesTraveled <= segment.cumulative) {
      const progress = segment.miles > 0
        ? (milesTraveled - segStart) / segment.miles
        : 0;
  ...
  // Past the last segment — return Mount Doom
 ...
 */
export function getJourneyPathPoints(): Array<{ name: string; x: number; y: number; z: number }> {
